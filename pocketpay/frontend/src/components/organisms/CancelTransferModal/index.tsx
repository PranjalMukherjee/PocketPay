    ...
    return (
      <StyledSelectedOptionStack>
        ...
        <TypographyComponent
          variant="body2"
          color={theme.palette.text.highEmphasis}
          children={`xxxx xxxx ${accountNumber}`}
        />
      </StyledSelectedOptionStack>
    );
  };
    ...
    <div>
      <StyledGrid>
        <Styledtext
          variant="body1"
          color={theme.palette.text.highEmphasis}
          children={`${CANCEL_TRANSFER}${props.transferId}`}
        />
        <TypographyComponent
          variant="caption"
          color={theme.palette.text.mediumEmphasis}
          children={WHERE_WOULD_YOU_LIKE_TO_REFUND}
        />
        {state.selectAccount ? (
          <>
            <StyledTextField
            ...
            <StyledForm variant="outlined">
              ...
              <Select
                ...
                renderValue={() => renderSelectedOptionStack(state.selectedOption)}
                labelId="demo-simple-select-label">
                {props.accounts.map((option) => {
                  return (
                    <MenuItem value={option} key={option}>
                      <Grid container direction="column">
                        <TypographyComponent variant="body2" children={props.username} />
                        <TypographyComponent
                          variant="caption"
                          children={`Ending in ${option}`}
